...
通过漫画编号下载漫画，打包为压缩包发送给你，支持 **Agent 自动判断触发**。

> 插件 id：`jm_downloader`　当前版本：`1.0.6`
...
| 版本 | 更新内容 |
|---|---|
| `1.0.6` | Agent 工具提示词中性化：去除「禁漫」「本子」等敏感措辞，统一改为「漫画」「漫画编号」，降低模型内容过滤拒答的概率；同步更新元数据描述 |
| `1.0.5` | 修复：将 jmcomic 下载目录固定到临时目录并自动清理，避免在 AstrBot 根目录残留章节目录 |
